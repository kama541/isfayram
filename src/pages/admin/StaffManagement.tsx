  ...
  return (
    <div className="p-8 space-y-8 font-sans max-w-7xl mx-auto">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Xodimlar Boshqaruvi</h1>
          <p className="text-slate-500 mt-1">Kassirlar va ofitsiantlarni qo'shish va parollar o'rnatish</p>
        </div>
        <button 
          onClick={() => {
      ...
      {isAdding && (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 mb-8 animation-fade-in">
          <h2 className="text-lg font-bold text-slate-800 mb-4">{editingId ? 'Xodimni tahrirlash' : 'Yangi xodim qo\'shish'}</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
            <div className="lg:col-span-2">
              <label className="block text-sm font-medium text-slate-700 mb-1">Ism familiyasi</label>
              <input
                type="text"
                required
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 font-mono tracking-widest"
                  placeholder="1234"
                />
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {employees.length === 0 ? (
          <div className="col-span-full p-12 text-center text-slate-500 bg-white rounded-2xl border border-slate-100 border-dashed">
            Hali xodimlar qo'shilmagan.
          </div>
        ) : employees.map(employee => (
          <div key={employee.id} className="bg-white border border-slate-100 rounded-2xl p-6 flex flex-col shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden">
            ...
            <div className="flex items-start gap-4 mb-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-slate-800 truncate">{employee.fullName}</h3>
            ...
            <div className="mt-auto bg-slate-50 rounded-xl p-3 flex justify-between items-center border border-slate-100">
              <div className="flex items-center gap-2">
                <KeyRound className="w-4 h-4 text-slate-400" />
                <span className="font-mono font-bold tracking-widest text-slate-700">{employee.pinCode}</span>
              </div>
